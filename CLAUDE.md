...
- **Chunk-metadata contract (ingestion):** every chunk carries eight fields in two groups.
  - *Provenance* — `product`, `source`, `doc_version`, `ingested_at`, `content_hash`. Stamped at load time. `ingested_at` is a **timezone-aware UTC ISO-8601 timestamp** (`datetime.now(timezone.utc)`, seconds precision), taken **once per run** so every chunk from one ingestion shares it and a run is identifiable in the store — never a naive local `date.today()`, which leaves the audit trail ambiguous when the job runs on a server in another zone. `respond` renders "Source: <source>, updated <doc_version>" with citations; observability logs these fields with each verify verdict so any answer can be traced to the exact document version that supported it. KB docs must carry a `Last updated: <Month Year>` line in the document body — any format (.docx, .pdf, .md, .txt); `doc_version` is parsed from the loaded text, so the pipeline is format-agnostic.
  - *Build config* — `embedding_model`, `chunk_size`, `chunk_overlap`. Stamped at chunk time; records the settings that produced the vector.
- **Section-aware chunking:** the splitter's first separator is a regex matching numbered headings (`\n\n\d+\.\s`), so chunks cut at section boundaries and fall back to paragraphs only when a section exceeds `CHUNK_SIZE`. Configured as `SPLIT_SEPARATORS` / `SPLIT_IS_REGEX` in `settings.py`. Measured across all four KB docs: chunks spanning more than one numbered section 9 → 1, chunks starting at a heading 6 → 41. The heading is strong topical signal in the embedding and gives `generate` a titled context unit rather than a fragment starting mid-sentence. Degrades gracefully — a KB with no numbered headings never matches the first separator and splits on paragraphs as before. Note `chunk_overlap` is an **upper bound**, not a guarantee: the splitter trims at separators, so consecutive chunks typically share fewer characters and share none at all across a clean break.
- **Store/settings drift guard:** the vector store is built from `settings.py`, so changing `EMBEDDING_MODEL`, `CHUNK_SIZE`, or `CHUNK_OVERLAP` after ingestion leaves the store and the config disagreeing. `retriever.py` reads one chunk per collection at startup (cached — one probe per process, not per query) and compares its build-config metadata against current settings: **`embedding_model` mismatch raises**, `chunk_size`/`chunk_overlap` mismatch warns. The raise is the important one — vectors from two different embedding models are not comparable, and when their dimensions happen to match, pgvector returns arbitrary chunks with no error while every downstream gate including `verify` still passes. Fix in both cases: re-run the offline job. Ingestion is therefore re-run whenever product docs change **or** any of those three settings changes.
